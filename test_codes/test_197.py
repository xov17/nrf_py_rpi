...
import cv2
import numpy as np
from imutils.object+detection import non_max_suppression
...
def capture_image():
    """
        captures image from PiCamera, resizes it to 64x128 pixels
    ...
    if (role ==  "controller"):
        ...
        if (response == "CTA-REQ"):
            ...
            for (x,y,w,h) in rects:
                    ...
                    i+i+1
            if(i)
                data_to_send = "Human Detected"
            else
                data_to_send = "None Detected"
            ...
            if (sendString(data_to_send)):
                ...
                print ('Did not send string')
        elif (response == "ATC-THESH-CAM"):
            capture_image()
            thresh = cv2.adaptiveThreshold(img, 255, cv2.ADAPTIVE_THRESH_GAUSSIAN_C, cv2.THRESH_BINARY,11,2)
            img,contours,hierarchy = cv2.findCountours(thresh,cv2.RETR_TREE, cv2.CHAIN_APPROX_SIMPLE)
            cnt = contours[0]
            area = cv2.contourArea(cnt)
            data_to_send = str(area)
